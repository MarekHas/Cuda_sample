...
void ConvertImageToGrayscaleOnCpu(unsigned char* imageRGBA, int width, int height)
{
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ImagePixel* ptrPixel = (ImagePixel*)&imageRGBA[y * width * 4 + 4 * x];
...
__global__ void ConvertImageToGrayscaleOnGpu(unsigned char* imageRGBA)
{
    uint32_t x = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t y = blockIdx.y * blockDim.y + threadIdx.y;
    uint32_t idx = y * blockDim.x * gridDim.x + x;

    ImagePixel* ptrPixel = (ImagePixel*)&imageRGBA[idx * 4];
...
int main(int argc, char** argv)
{
    ...
    if (width % 32 || height % 32)
    {
        ...
        std::cout << "image dimensions is not dividable by 32!";
        return -1;
    }
    
    //// Process image on cpu
    //std::cout << "Processing image...";
    //ConvertImageToGrayscaleOnCpu(imageData, width, height);
    //std::cout << " DONE cpu processing" << std::endl;
    ...
    ConvertImageToGrayscaleOnGpu << <gridSize, blockSize >> > (ptrImageDataGpu);
    ...
    fileNameOut = fileNameOut.substr(0, fileNameOut.find_last_of('.')) + "_grayscale.png";
